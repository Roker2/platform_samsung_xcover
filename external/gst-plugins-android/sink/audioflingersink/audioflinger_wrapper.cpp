  ...
  int channelCount, uint32_t sampleRate, int bufferCount)
{
  ...
  enum AudioSystem::audio_channels channel = AudioSystem::CHANNEL_OUT_STEREO;
  switch(channelCount) {
    case 1:
      channel = AudioSystem::CHANNEL_OUT_MONO;
      break;
    case 2:
    default:
      channel = AudioSystem::CHANNEL_OUT_STEREO;
      break;
  }
#endif
  ...
  if(AUDIO_FLINGER_DEVICE_TRACK(handle)) {
    ...
    // buffer size
#if PLATFORM_SDK_VERSION < 5
    status = AUDIO_FLINGER_DEVICE_TRACK(handle)->set(AudioSystem::DEFAULT, sampleRate, 
        format, channelCount, bufferCount);
#else 
    status = AUDIO_FLINGER_DEVICE_TRACK(handle)->set(AudioSystem::DEFAULT, sampleRate, 
        format, channel, bufferCount);
#endif
    GST_PLAYER_DEBUG("Set AudioTrack, status: %d, sampleRate: %d, "
        "channelCount: %d, bufferCount: %d, frameCount: %d\n", status, sampleRate, 
        channelCount, bufferCount, AUDIO_FLINGER_DEVICE_TRACK(handle)->frameCount());
  }
  else if(AUDIO_FLINGER_DEVICE_SINK(handle).get()) {
    status = AUDIO_FLINGER_DEVICE_SINK(handle)->open(sampleRate, channelCount, 
        format, bufferCount);
    GST_PLAYER_DEBUG("Set AudioSink, status: %d, sampleRate: %d," 
        "channelCount: %d, bufferCount: %d, frameCount: %d\n", status, sampleRate, 
        channelCount, bufferCount, AUDIO_FLINGER_DEVICE_SINK(handle)->frameCount());
  }
...
int audioflinger_device_outputFrameCount()
{
    int frame_count = 0;
    AudioSystem::getOutputFrameCount(&frame_count, AudioSystem::DEFAULT);
    return frame_count;
}

int audioflinger_device_outputSampleRate()
{
    int sample_rate = 44100;
    AudioSystem::getOutputSamplingRate(&sample_rate, AudioSystem::DEFAULT);
    return sample_rate;
}
