...
static void gst_audioflinger_sink_pause (GstAudioSink * asink);
...
gst_audioflinger_sink_class_init (GstAudioFlingerSinkClass * klass)
{
  ...
  gstaudiosink_class->reset = GST_DEBUG_FUNCPTR (gst_audioflinger_sink_pause);
...
gst_audioflinger_sink_init (GstAudioFlingerSink * audioflinger_sink)
{
  ...
  gst_base_audio_sink_set_drift_tolerance(audioflinger_sink, 400*1000); //to fix wma playback discontinue issue. 
...
#if 0
  /* The base audio sink's buffer time is 200ms, latency time is 10ms. We have no reason to modify
     it. And if we set the latency_time as 50ms, we will hear a short pause after transition from
     PAUSED to PLAYING state.*/
...
gst_audioflinger_sink_prepare (GstAudioSink * asink, GstRingBufferSpec * spec)
{
  ...
  //Here, according to the Audiotrack's latency, we set the proper ringbuffer size, this can avoid ringbuffer data underrun at the beginning
  gint64 modified_buf_time = audioflinger_device_latency(audioflinger->audioflinger_device) * (GST_MSECOND / GST_USECOND);  //if error, audioflinger_device_latency() return -1
  if(modified_buf_time < (gint64)(300*GST_MSECOND/GST_USECOND)) {
    modified_buf_time = 300*GST_MSECOND/GST_USECOND;  //sometimes calculated value from audioflinger_device_latency() probably small, since default value of spec->buffer_time is 200ms, we clip buffer_time to [300ms, xxx)
  }
  spec->buffer_time = modified_buf_time;
  spec->segtotal = spec->buffer_time / spec->latency_time;
  GST_DEBUG_OBJECT (audioflinger,"re-set the ringbuffer size to %llu(us) to align with Audiotrack latency",spec->buffer_time);
...
#if 0
  latency = audioflinger_device_frameCount (audioflinger->audioflinger_device) * 1000 / audioflinger_device_outputSampleRate();
  GST_INFO_OBJECT(audioflinger, "set audio render delay %d ms", latency);
  gst_base_sink_set_render_delay (GST_BASE_SINK (asink), latency * GST_MSECOND);
#endif
  return TRUE;
...
gst_audioflinger_sink_write (GstAudioSink * asink, gpointer data, guint length)
{
  ...
  GST_DEBUG_OBJECT (audioflinger, "write length=%d", length);
  ...
  GST_DEBUG_OBJECT (audioflinger, "written=%u", ret);
...
gst_audioflinger_sink_delay (GstAudioSink * asink)
{
    ...
    audioflinger = GST_AUDIOFLINGERSINK (asink);
    gstbaseaudiosink = (GstBaseAudioSink *) asink;

    latency = (guint)audioflinger_device_latency(audioflinger->audioflinger_device);
    //convet audiosink's latency (ms) to ringbuffer samples
    latency = latency * (gstbaseaudiosink->ringbuffer->spec.rate/1000);
    //latency = ((latency*1000)/gstbaseaudiosink->ringbuffer->spec.latencytime)*(gstbaseaudiosink->ringbuffer.segsize / gstbaseaudiosink->ringbuffer.bytes_per_sample);
    //GST_LOG_OBJECT (audioflinger, "query latency = %u (samples)",latency);
    return latency;
}

static void
gst_audioflinger_sink_pause (GstAudioSink * asink)
{
  GstAudioFlingerSink *audioflinger;
  ...
  if (audioflinger->audioflinger_device != NULL) {
    GST_DEBUG_OBJECT (audioflinger, "pause flinger device");
    audioflinger_device_pause(audioflinger->audioflinger_device);
  }
}
...
{
  return gst_element_register (plugin, "audioflingersink", GST_RANK_PRIMARY+1,
