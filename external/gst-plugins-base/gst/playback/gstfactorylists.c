...
//marvell special 2011.1
/* function used to sort element features. We first sort on the rank, then
 * on the element name (to get a consistent, predictable list) */
static gint
compare_ranks_mrvl_lower_cnm (GValue * v1, GValue * v2)
{
  gint diff;
  const gchar *rname1, *rname2;
  GstPluginFeature *f1, *f2;
  ...
  //for HW decoder, artificially lower its rank
  if(0 == strncmp(rname1, "cnmdec_", 7) || 0 == strncmp(rname1, "ippsimplebin", 12)) {
    rank1 = GST_RANK_PRIMARY - 1;
  }
  if(0 == strncmp(rname2, "cnmdec_", 7) || 0 == strncmp(rname2, "ippsimplebin", 12)) {
    rank2 = GST_RANK_PRIMARY - 1;
  }
...
static gint
compare_ranks_mrvl_lower_vmeta (GValue * v1, GValue * v2)
{
  gint diff;
  const gchar *rname1, *rname2;
  GstPluginFeature *f1, *f2;
  ...
  //for HW decoder, artificially lower its rank
  if(0 == strncmp(rname1, "vmetadec", 8)) {
    rank1 = GST_RANK_PRIMARY - 1;
  }
  if(0 == strncmp(rname2, "vmetadec", 8)) {
    rank2 = GST_RANK_PRIMARY - 1;
  }
...
static gint
compare_ranks_mrvl_lower_cnmvmeta (GValue * v1, GValue * v2)
{
  gint diff;
  const gchar *rname1, *rname2;
  GstPluginFeature *f1, *f2;
  ...
  //for HW decoder, artificially lower its rank
  if(0 == strncmp(rname1, "cnmdec_", 7) || 0 == strncmp(rname1, "ippsimplebin", 12) || 0 == strncmp(rname1, "vmetadec", 8)) {
    rank1 = GST_RANK_PRIMARY - 1;
  }
  if(0 == strncmp(rname2, "cnmdec_", 7) || 0 == strncmp(rname2, "ippsimplebin", 12) || 0 == strncmp(rname2, "vmetadec", 8)) {
    rank2 = GST_RANK_PRIMARY - 1;
  }
...
/**
 * gst_factory_list_get_elements_mrvl:
 * @type: a #GstFactoryListType
 *
 * Get a sorted list of factories of @type.
 *
 * Returns: a #GValueArray of #GstElementFactory elements. Use
 * g_value_array_free() after usage.
 * called when hardware acceleration is disabled
 */
GValueArray *
gst_factory_list_get_elements_mrvl (GstFactoryListType type, int option)
{
  GValueArray *result;
  GList *walk, *list;
  FilterData data;
  ...
  if(option == 1) {
    //lower CNM
    g_value_array_sort (result, (GCompareFunc) compare_ranks_mrvl_lower_cnm);
  }else if(option == 2) {
    //lower vMeta
    g_value_array_sort (result, (GCompareFunc) compare_ranks_mrvl_lower_vmeta);
  }else if(option == 3) {
    //lower CNM and vMeta
    g_value_array_sort (result, (GCompareFunc) compare_ranks_mrvl_lower_cnmvmeta);
  }
