...
eos:
  {
    ...
    sent_eos_suc = gst_asf_demux_send_event_unlocked (demux, gst_event_new_eos ());
    sent_eos = TRUE;
...
pause:
  {
    ...
    //2011.09.21. If the asf file is cuted to very short, before demuxer send any data to downstream, it probably has reached EOS of file, then the EOS event won't be handled by downstream. Therefore, need to fire error.
    else if(!sent_eos_suc) {
        if (GST_FLOW_IS_FATAL (flow) || flow == GST_FLOW_NOT_LINKED) {
            GST_ELEMENT_ERROR (demux, STREAM, FAILED,
                (_("Internal data stream error.")),
                ("streaming stopped, reason %s, ASFfile probably be cuted too short.", gst_flow_get_name (flow)));
        }
    }
    ...
    guint64 size)
{
  ...
  switch (stream_type) {
    ...
    case ASF_STREAM_VIDEO:{
      ...
      //Add more check for some corrupted stream
      if(video_format_object.width > 4096 || video_format_object.height > 4096) {
       GST_ELEMENT_ERROR (demux, STREAM, DEMUX, (NULL), ("Not support so big video: width %d, height %d, probably corrupted stream!", video_format_object.width, video_format_object.height));
       return NULL;
      }
      if(video_format_object.size>40 && video_format_object.size-40 > size) {
        goto not_enough_data;
      }
