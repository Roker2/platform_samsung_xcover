    ...
    guint num, GstAviIndexEntry * entry)
{
  ...
  if (G_UNLIKELY (stream->idx_n >= stream->idx_max)) {
    ...
    if (idx_max == 0) {
      ...
      //2011.04.26, we adjust increase step according to index num. For 10000 num index, we set increase step to about 20k; for 200000 num index, we set increase step to near 400k.
      guint increase = num<<1;
      if(increase < 8192) {
        increase = 8192;
      }else if(increase > 384*1024) {
        increase = 384*1024;
      }
      idx_max += increase / sizeof (GstAviIndexEntry);
