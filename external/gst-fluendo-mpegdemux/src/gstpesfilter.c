...
gst_pes_filter_parse (GstPESFilter * filter)
{
  ...
  } else if (filter->length == 0 && filter->start_code == ID_PADDING_STREAM) {
    GST_DEBUG ("id 0x%02x, padding stream with length 0, skip start code", 
        filter->id);
    goto lost_sync;
  } else {
    filter->unbounded_packet = FALSE;

    if (G_UNLIKELY (filter->gather_pes && avail < filter->length + 6)) {
  ...
  /* PTS and DTS, never for mpeg2 */
    ...
    if ((flags & 0x10)) {
      ...
      GST_DEBUG ("ES Rate found %u", es_rate);
    ...
    if ((flags & 0x08)) {
      ...
      GST_DEBUG ("DSM trick mode found, flags 0x%02x", trick_mode_flags);
    ...
    if ((flags & 0x04)) {
      GST_DEBUG ("additional copy info, flags 0x%02x", *data);
    }
    /* PES_CRC_flag  */
    if ((flags & 0x02)) {
      GST_DEBUG ("PES_CRC");
    ...
    if ((flags & 0x01)) {
      ...
      header_data_length -= 1;
      datalen -= 1;
      GST_DEBUG ("PES_extension, flags 0x%02x", flags);
      /* PES_private_data_flag */
      if ((flags & 0x80)) {
        GST_DEBUG ("PES_private_data_flag");
        data += 16;
      ...
      if ((flags & 0x40)) {
        ...
        GST_DEBUG ("pack_header_field_flag, pack_field_length %d",
            pack_field_length);
      ...
      if ((flags & 0x20)) {
        GST_DEBUG ("program_packet_sequence_counter_flag");
        data += 2;
      ...
      if ((flags & 0x01)) {
        guint8 PES_extension_field_length = 0;
        if (datalen < 1)
          goto need_more_data;

        PES_extension_field_length = *data++;
        GST_DEBUG ("PES_extension_flag_2, len %d",
            PES_extension_field_length & 0x7f);
        if (PES_extension_field_length == 0x81) {
          GST_DEBUG ("substream id 0x%02x", *data);
        }
        else if (((PES_extension_field_length & 0x7f) + 1) > datalen) {
          GST_DEBUG ("not enough data parsing PES_extension_field, %d > %d",
              (PES_extension_field_length & 0x7f) + 1, datalen);          
          goto need_more_data;
        }
...
push_out:
  {
    ...
    if (G_LIKELY (filter->length > 0 || filter->unbounded_packet))
