  ...
  switch (stream_type) {
    ...
    case ST_BD_AUDIO_AC3:
    {
      GstFluTSStream *PMT_stream =
          gst_fluts_demux_get_stream_for_PID (stream->demux, stream->PMT_pid);
    ...
    GstFluTSStream * PTS_stream)
{
  ...
  if (PCR_stream->base_PCR == -1 && PCR_stream->last_PCR == -1) {
    if (PTS_stream->stream_type == ST_PRIVATE_DATA)
  ...
  if (!demux->force_pcr_segment && pts < base_PCR) {
    ...
    GstBuffer * buffer, GstFluTSStream * stream)
{
  ...
  if (first && filter->pts != -1) {
    ...
    if ((stream->last_time > 0 && stream->last_time < time &&
      ...
      /* check for wraparounds */
        ...
        if (ABS (time - MAX_MPEG_PTS - stream->last_time) < GST_SECOND) {
          GST_DEBUG_OBJECT (demux,
  ...
  GST_DEBUG_OBJECT (demux, "setting PTS to (%" G_GUINT64_FORMAT ") time: %"
      GST_TIME_FORMAT " first buffer: %d base_time: %"
      GST_TIME_FORMAT, pts, GST_TIME_ARGS (time), first,
  ...
  if (stream->next_buffer_is_access_point || first) {
    stream->next_buffer_is_access_point = FALSE;
  } else {
    GST_BUFFER_FLAG_SET (buffer, GST_BUFFER_FLAG_DELTA_UNIT);
  }

  GST_DEBUG_OBJECT (srcpad, "pushing buffer with PTS %" GST_TIME_FORMAT \
      " size %u", GST_TIME_ARGS (GST_BUFFER_TIMESTAMP (buffer)),
    ...
    const guint8 * data, guint data_len, guint * consumed)
{
  ...
  if (length > 0) {
    ...
    if (flags & 0x40) {
      ...
      stream->next_pes_is_access_point = TRUE;
    }
    /* PCR_flag */
    if (flags & 0x10) {
      guint32 pcr1;
  ...
  for (i = 0; i < FLUTS_MAX_PID + 1; i++) {
    ...
    if (stream != NULL && stream->PID_type == PID_TYPE_ELEMENTARY) {
      ...
      if (!found) {
        ...
        stream->next_buffer_is_access_point = FALSE;
        stream->next_pes_is_access_point = FALSE;
        stream->base_PCR = -1;
        stream->last_PCR = -1;
        stream->last_PCR_difference = -1;
        stream->base_time = 0;
    ...
    switch (stream->PID_type) {
      ...
      case PID_TYPE_ELEMENTARY:
      {
        if (payload_unit_start_indicator) {
          ...
          stream->pes_buffer_in_sync = FALSE;
          stream->pes_buffer_overflow = FALSE;
          stream->next_buffer_is_access_point =
              stream->next_pes_is_access_point;
          stream->next_pes_is_access_point = FALSE;
        }
