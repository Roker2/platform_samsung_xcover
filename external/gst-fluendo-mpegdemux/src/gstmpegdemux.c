...
//marvell modification 2010.12.28
#define MRVL_GET_UINT32_FROM_BUF_LE(buf)	(((buf)[0]) | (((buf)[1])<<8) | (((buf)[2])<<16) | (((buf)[3])<<24))
...
static GstPadLinkReturn gst_flups_demux_src_link (GstPad * pad, GstPad * peer);
...
static GstPadClass *streamparent_class = NULL;

/*static guint gst_flups_demux_signals[LAST_SIGNAL] = { 0 };*/
static void
gst_flupsstream_dispose (GObject * object)
{
  /* Chaining up */
  if (G_OBJECT_CLASS (streamparent_class)->dispose != NULL) {
    G_OBJECT_CLASS (streamparent_class)->dispose (object);
  }
}

static void
gst_flupsstream_class_init (GstFluPSStreamClass * klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  streamparent_class = g_type_class_ref (GST_TYPE_PAD);

  gobject_class->dispose = gst_flupsstream_dispose;
}

static void
gst_flupsstream_init (GstFluPSStream * stream, GstFluPSStreamClass * g_class)
{
  gst_pad_set_link_function (GST_PAD (stream),
      GST_DEBUG_FUNCPTR (gst_flups_demux_src_link));
...
GType
gst_flupsstream_get_type (void)
{
  static GType stream_type = 0;

  if (!stream_type) {
    static const GTypeInfo stream_info = {
      sizeof (GstFluPSStreamClass),
      NULL,
      NULL,
      (GClassInitFunc) gst_flupsstream_class_init,
      NULL,
      NULL,
      sizeof (GstFluPSStream),
      0,
      (GInstanceInitFunc) gst_flupsstream_init,
    };

    stream_type = g_type_register_static (GST_TYPE_PAD, "GstFluPSStream",
        &stream_info, 0);
  }
  return stream_type;
}
...
gst_flups_demux_reset (GstFluPSDemux * demux)
{
  ...
  for (i = 0; i < GST_FLUPS_DEMUX_MAX_STREAMS; i++) {
    ...
    if (stream != NULL) {
      if (GST_PAD (stream))
        gst_element_remove_pad (GST_ELEMENT_CAST (demux), GST_PAD (stream));
...
gst_flups_demux_create_stream (GstFluPSDemux * demux, gint id, gint stream_type)
{
  ...
  stream = g_object_new (GST_TYPE_FLUPSSTREAM, "name", name, "direction",
      GST_PAD_SRC, "template", template, NULL);
  ...
  if (stream) {
    ...
    GST_DEBUG_OBJECT (demux, "create pad %s, caps %" GST_PTR_FORMAT, name,
        caps);
  } else {
    GST_WARNING_OBJECT (demux, "failed creating a stream object");
  }
...
gst_flups_demux_get_stream (GstFluPSDemux * demux, gint id, gint type)
{
  ...
  if (stream == NULL) {
    ...
    demux->streams[id] = gst_object_ref (stream);
    demux->streams_found[demux->found_count++] = gst_object_ref (stream);

    gst_pad_set_active (GST_PAD (stream), TRUE);
    gst_element_add_pad (GST_ELEMENT (demux), GST_PAD (stream));
    ...
    GstBuffer * buf)
{
  ...
  if (G_UNLIKELY (stream->need_segment)) {
    ...
    GST_INFO_OBJECT (demux, "sending new segment: rate %g applied_rate %g "
        ...
        GST_TIME_ARGS (time), GST_PAD (stream));
  ...
  /* caps */
  gst_buffer_set_caps (buf, GST_PAD_CAPS (GST_PAD (stream)));
  GST_BUFFER_TIMESTAMP (buf) = timestamp;

  /* When possible, update position in the segment */
  ...
  if (demux->src_segment.last_stop != GST_CLOCK_TIME_NONE) {
    ...
    if (stream->last_ts == GST_CLOCK_TIME_NONE || stream->last_ts < new_time) {
      ...
      g_print ("last_ts update on pad %s to time %" GST_TIME_FORMAT "\n",
          GST_PAD_NAME (GST_PAD (stream)), GST_TIME_ARGS (cur_scr_time));
#endif
      stream->last_ts = new_time;
    }
    
    /* Handling of sparse streams with segment updates is not implemented for
     * reverse playback, just skip that function for now in that case. */
    if (G_LIKELY (demux->src_segment.rate > 0.0)) {
  ...
  result = gst_pad_push (GST_PAD (stream), buf);
...
gst_flups_demux_send_event (GstFluPSDemux * demux, GstEvent * event)
{
  ...
  for (i = 0; i < count; i++) {
    ...
    if (stream && !stream->notlinked) {
      ...
      if (!gst_pad_push_event (GST_PAD (stream), event)) {
...
gst_flups_demux_do_seek (GstFluPSDemux * demux, GstSegment * seeksegment)
{
  ...
  GST_DEBUG_OBJECT (demux, "doing search for SCR %" G_GUINT64_FORMAT, scr);
  ...
  if (demux->last_scr > demux->last_pts) {
    ...
    GST_DEBUG_OBJECT (demux, "corrected SCR %" G_GUINT64_FORMAT, scr);
  }
...
#endif
  /* Initial stage: interpolate search */
  offset = MIN (gst_util_uint64_scale (scr - demux->first_scr, scr_rate_n,
      scr_rate_d), demux->sink_segment.stop);
  GST_DEBUG_OBJECT (demux, "interpolated offset %" G_GUINT64_FORMAT \
      " with SCR rate %" G_GUINT64_FORMAT " / %" G_GUINT64_FORMAT,
      offset, scr_rate_n, scr_rate_d);
  soffset = offset;
  found = gst_flups_demux_scan_forward_ts (demux, &soffset, SCAN_SCR, &fscr);
  if (!found) {
    soffset = offset;
    found = gst_flups_demux_scan_backward_ts (demux, &soffset, SCAN_SCR, &fscr);
    GST_DEBUG_OBJECT (demux, "backward scan found SCR %" G_GUINT64_FORMAT \
        " at offset %" G_GUINT64_FORMAT, fscr, soffset);
  } else {
    GST_DEBUG_OBJECT (demux, "forward scan found SCR %" G_GUINT64_FORMAT \
        " at offset %" G_GUINT64_FORMAT, fscr, soffset);
  }
  offset = soffset;
  
  /* Intermediate stage: approximate search */
  for (i = 8; i > 0; i>>=1) {
    GST_DEBUG_OBJECT (demux, "doing approximate search with factor %d", i);
    while (found && fscr < scr) {
      offset += i * BLOCK_SZ;
      found = gst_flups_demux_scan_forward_ts (demux, &offset, SCAN_SCR, &fscr);
      GST_LOG_OBJECT (demux, "forward scan found SCR %" G_GUINT64_FORMAT \
          " at offset %" G_GUINT64_FORMAT, fscr, offset);
    }

    while (found && fscr > scr && offset > 0) {
      offset -= i * BLOCK_SZ;
      found = gst_flups_demux_scan_backward_ts (demux, &offset, SCAN_SCR, &fscr);
      GST_LOG_OBJECT (demux, "backward scan found SCR %" G_GUINT64_FORMAT \
          " at offset %" G_GUINT64_FORMAT, fscr, offset);    
    }
  }
  /* Final stage: fine search */
  while (found && fscr < scr) {
    offset += 1;
    found = gst_flups_demux_scan_forward_ts (demux, &offset, SCAN_SCR, &fscr);
    GST_LOG_OBJECT (demux, "forward scan found SCR %" G_GUINT64_FORMAT \
...
gst_flups_demux_src_link (GstPad * pad, GstPad * peer)
{
  GstFluPSStream *stream = GST_FLUPSSTREAM (pad);
  stream->notlinked = FALSE;
  stream->discont = TRUE;
  stream->need_segment = TRUE;
...
gst_flups_demux_parse_pack_start (GstFluPSDemux * demux)
{
  ...
  scr1 = GUINT32_FROM_BE (MRVL_GET_UINT32_FROM_BUF_LE(data));
  scr2 = GUINT32_FROM_BE (MRVL_GET_UINT32_FROM_BUF_LE(data+4));
  ...
  if ((*data & 0xc0) == 0x40) {
	...
	next32 = (GUINT32_FROM_BE (MRVL_GET_UINT32_FROM_BUF_LE(data)));
  ...
  /* scr adjusted is the new scr found + the collected adjustment */
  ...
  if (G_LIKELY (demux->current_scr != G_MAXUINT64)) {
    ...
    if (G_UNLIKELY (diff > CLOCK_FREQ)) {
      ...
      /* In pull mode adjust the duration too and report it changed */      
      if (demux->random_access) {
        gst_segment_set_duration (&demux->src_segment, GST_FORMAT_TIME,
            MPEGTIME_TO_GSTTIME (demux->last_pts - demux->first_pts + 
                demux->scr_adjust));
        GST_LOG_OBJECT (demux, "new adjusted duration is %" GST_TIME_FORMAT,
            GST_TIME_ARGS (demux->src_segment.duration));
...
gst_flups_demux_parse_sys_head (GstFluPSDemux * demux)
{
...

  {
    ...
    for (i = 0; i < stream_count; i++) {
      ...
      GST_DEBUG_OBJECT (demux, "stream_id %02x", stream_id);
    ...
    SCAN_MODE mode, guint64 * rts)
{
  ...
  /* Skip the system header if found */ 
...
gst_flups_sink_get_duration (GstFluPSDemux * demux)
{
  ...
  if (demux->first_pts >= demux->last_pts) {
    guint64 segment, last_pts, last_pts_offset = 0;

    GST_DEBUG_OBJECT (demux, "Wrong last PTS detected, try to get better "
        "values for last SCR and last PTS");
    segment = offset - demux->sink_segment.start;
    last_pts = demux->last_pts;
    /* Authorize ourselves 8 tries */
    for (i = 0; i < 8; i++) {
      segment /= 2;
      if (demux->first_pts >= last_pts) {
      ...
      /* Now search. */
      GST_LOG_OBJECT (demux, "Trying last PTS scan at position %" \
          G_GUINT64_FORMAT " (iteration %u)", offset, i);
      /* We could try to optimize search direction using previous result */
      gst_flups_demux_scan_forward_ts (demux, &offset, SCAN_PTS, &last_pts);
      if (demux->first_pts < last_pts) {
        demux->last_pts = last_pts;
        last_pts_offset = offset;
        GST_DEBUG_OBJECT (demux, "Found a better last PTS value %" \
            GST_TIME_FORMAT " at %" G_GUINT64_FORMAT,
            GST_TIME_ARGS (MPEGTIME_TO_GSTTIME (last_pts)), offset);
      }
    }

    if (G_LIKELY (last_pts_offset))
      offset = last_pts_offset;

       /* and start looking for the SCR from that point */
...
gst_flups_demux_plugin_init (GstPlugin * plugin)
{
  if (!gst_element_register (plugin, "flupsdemux",
          GST_RANK_PRIMARY+1, GST_TYPE_FLUPS_DEMUX))
    return FALSE;
