...
gst_mjpegdec_init_members(GstMJPEGDec *mjpeg)
{
	...
	mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.nDesiredColor = JPEG_YUV411;

	mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.nAlphaValue = 255;
	mjpeg->outputWidth = 0;
	mjpeg->outputHeight = 0;
	mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.srcROI.x      = 0;
	mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.srcROI.y      = 0;
	mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.srcROI.width  = 4096;
	mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.srcROI.height = 4096;
	mjpeg->jpegDecPar.nModeFlag = IPP_JPEG_ROIDEC_STD;

	mjpeg->bCapSet = 0;
	mjpeg->UserDefinedOutputSizeFlag = FALSE;
	mjpeg->UserDefinedSrcROIFlag = FALSE;
	...
	mjpeg->dec_inbuf = NULL;
	mjpeg->dec_inbuf_sz = 0;
	return;
}
...
static IppCodecStatus gst_mjpegdec_codec_init(GstMJPEGDec * mjpeg)
{
	...
	ret = DecoderInitAlloc_JPEG(&(mjpeg->srcBitStream), &(mjpeg->dstPicture), mjpeg->pCBTable, &(mjpeg->pJPEGDecState));
	...
	mjpeg->inputWidth = mjpeg->dstPicture.picWidth;
	mjpeg->inputHeight = mjpeg->dstPicture.picHeight;
...
gst_mjpegdec_set_check_param(GstMJPEGDec* mjpeg, IppJPEGROIDecParam *pROIDecPar)
{
	debug_printf("enter %s \n", __FUNCTION__);
	/*********** Check and set correct size parameter ***********/
	if(mjpeg->UserDefinedSrcROIFlag == FALSE) {
		//set ROI according to input picture's dimension
		pROIDecPar->srcROI.x = 0;
		pROIDecPar->srcROI.y = 0;
		pROIDecPar->srcROI.width = mjpeg->inputWidth;
		pROIDecPar->srcROI.height = mjpeg->inputHeight;
	}else{
		//check user defined ROI
		/*check bad argument.*/
		if(pROIDecPar->srcROI.x >= mjpeg->inputWidth || pROIDecPar->srcROI.y >= mjpeg->inputHeight) {
			GST_ERROR("ROI start point exceed picture!");
			return GST_FLOW_ERROR;
		}

		/* reset ROI into bundary of picture if exceed the picture.*/
		if(pROIDecPar->srcROI.x + pROIDecPar->srcROI.width > mjpeg->inputWidth){
			pROIDecPar->srcROI.width = mjpeg->inputWidth - pROIDecPar->srcROI.x;
		}
		if(pROIDecPar->srcROI.y + pROIDecPar->srcROI.height > mjpeg->inputHeight){
			pROIDecPar->srcROI.height = mjpeg->inputHeight - pROIDecPar->srcROI.y;
		}
	}


	/* If user does not set output size, set as original. */
	if( mjpeg->UserDefinedOutputSizeFlag == FALSE ){
		mjpeg->outputWidth = pROIDecPar->srcROI.width;
		mjpeg->outputHeight = pROIDecPar->srcROI.height;
	}

	if(G_UNLIKELY(mjpeg->outputWidth > MAX_OUTPUT_WIDTH)) {
		GST_WARNING_OBJECT(mjpeg, "output width %d exceeds the max output width, reset to %d", mjpeg->outputWidth, MAX_OUTPUT_WIDTH);
		mjpeg->outputWidth = MAX_OUTPUT_WIDTH;
	}
	if(G_UNLIKELY(mjpeg->outputHeight > MAX_OUTPUT_HEIGHT)) {
		GST_WARNING_OBJECT(mjpeg, "output height %d exceeds the max output height, reset to %d", mjpeg->outputHeight, MAX_OUTPUT_HEIGHT);
		mjpeg->outputHeight = MAX_OUTPUT_HEIGHT;
	}

	mjpeg->outputWidth = (mjpeg->outputWidth + 1) & 0xFFFFFFFE;
	mjpeg->outputHeight = (mjpeg->outputHeight + 1) & 0xFFFFFFFE;


	pROIDecPar->nDstWidth = mjpeg->outputWidth;
	pROIDecPar->nDstHeight = mjpeg->outputHeight;
	
	pROIDecPar->iLPCutFreq = -1;

	/*********** set output format ***********/
	switch(mjpeg->dstPicture.picFormat+JPEG_YUV444) {
	case JPEG_YUV444:
		mjpeg->outGstBufDataFormat = JPEG_YUV444;
		break;
	case JPEG_YUV422:
		...
		break;
	case JPEG_YUV422I:
		mjpeg->outGstBufDataFormat = JPEG_YUV422I;
		break;
	case JPEG_YUV411:
		mjpeg->outGstBufDataFormat = JPEG_YUV411;
		break;
	...
	mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.nDesiredColor = mjpeg->dstPicture.picFormat+JPEG_YUV444;
	if(mjpeg->bForce_422to420 != 0) {
		if(mjpeg->dstPicture.picFormat+JPEG_YUV444 == JPEG_YUV422) {
			//if input jpeg is yuv422, convert it to 420
			mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.nDesiredColor = JPEG_YUV411;
		}
	}
...
static GstFlowReturn gst_mjpegdec_init_dst(GstMJPEGDec * mjpeg, IppPicture * DstPicture)
{
	...
	if(mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.nDesiredColor == JPEG_GRAY8) {
...
gst_mjpegdec_chain (GstPad * pad, GstBuffer * buffer)
{
	...
	IppJPEGROIDecParam *pROIDecPar = &DecoderPar->UnionParamJPEG.roiDecParam;
...
#define MAX_PADDEDBYTE_LENGTH	128*1024	//for qtdemuxer and mov file, there are probably a lot of padding bytes. so define MAX_PADDEDBYTE_LENGTH as a big value
...

	{
		...
		//prepare decoder input buffer
		if(mjpeg->dec_inbuf_sz < SrcBitStream->bsByteLen + IPP_EXTRA_TAIL_BYTES) {
			if(mjpeg->dec_inbuf) {
				mjpeg->dec_inbuf = (unsigned char*)realloc(mjpeg->dec_inbuf, SrcBitStream->bsByteLen + IPP_EXTRA_TAIL_BYTES);
			}else{
				mjpeg->dec_inbuf = (unsigned char*)malloc(SrcBitStream->bsByteLen + IPP_EXTRA_TAIL_BYTES);
			}
			if(mjpeg->dec_inbuf == NULL) {
				mjpeg->dec_inbuf_sz = 0;
				GST_ERROR_OBJECT(mjpeg, "Couldn't allocate decoder input buffer(sz %d)", SrcBitStream->bsByteLen + IPP_EXTRA_TAIL_BYTES);
				goto chain_error;
			}else{
				mjpeg->dec_inbuf_sz = SrcBitStream->bsByteLen + IPP_EXTRA_TAIL_BYTES;
			}
		}
		memcpy(mjpeg->dec_inbuf, SrcBitStream->pBsBuffer, SrcBitStream->bsByteLen);
		SrcBitStream->pBsBuffer = SrcBitStream->pBsCurByte = mjpeg->dec_inbuf;

		if( gst_mjpegdec_codec_init(mjpeg) != IPP_STATUS_NOERR ) {
			goto chain_error;
		}

		if(gst_mjpegdec_set_check_param(mjpeg, pROIDecPar) != GST_FLOW_OK) {
			goto chain_error;
		}
	...
	decoding_ret = Decode_JPEG(SrcBitStream, NULL, DstPicture, DecoderPar, mjpeg->pJPEGDecState);
...
#define FILTER_COEFEXP	2
	...
	case IPP_STATUS_JPEG_EOF:
		debug_printf(" %s: JPEG codec reach EOF, decode OK. \n",__FUNCTION__);
		break;
	...
	GstElement *element, GstStateChange transition)
{
	...
	//printf("mjpeg state change trans from %d to %d, skip strategy is %d, sz %d %d\n", transition>>3, transition&7, mjpeg->skip_strategy, mjpeg->inputWidth, mjpeg->inputHeight);
	debug_printf("enter %s \n", __FUNCTION__);
	...
	case GST_STATE_CHANGE_PAUSED_TO_READY:
		...
		printf("resize dst width is %d, resize dst height is %d\n", mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.nDstWidth, mjpeg->jpegDecPar.UnionParamJPEG.roiDecParam.nDstHeight);
