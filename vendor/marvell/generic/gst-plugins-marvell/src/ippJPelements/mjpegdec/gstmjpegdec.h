...
struct _GstMJPEGDec
{
	...
	int							outputHeight;
	int							inputWidth; //input picture's total dimension.
	int							inputHeight;

	int							bCapSet;

	gboolean					UserDefinedOutputSizeFlag;
	gboolean					UserDefinedSrcROIFlag;
	...
	unsigned char*				dec_inbuf;	//jpegdec need IPP_EXTRA_TAIL_BYTES bytes following compressed data, therefore, need additional input buffer which larger than length of compressed data
	int							dec_inbuf_sz;
