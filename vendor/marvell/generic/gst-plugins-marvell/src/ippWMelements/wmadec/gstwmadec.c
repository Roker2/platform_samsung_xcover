...
gst_wmadec_sinkpad_setcaps (GstPad *pad, GstCaps *caps)
{
	...
	name = gst_structure_get_name(str);
	if(0 == strcmp(name, "audio/x-ac3")) {
		wma->bForceForbidFormat = 1;
		return TRUE;
	}
...
gst_wmadec_chain (GstPad *pad, GstBuffer *buf)
{
	...
	if(wma->bForceForbidFormat != 0) {
		gst_buffer_unref(buf);
		GST_ELEMENT_ERROR(wma, STREAM, DECODE, (NULL), ("Terminate decoding, Not support this stream, reason %d", wma->bForceForbidFormat));
