...
#define IPPGST_FORCEFORBID_WMV78	//define it will forbid wmv7 & wmv8(also named WMV1 and WMV2)
#ifndef IPPGST_FORCEFORBID_WMV78
...
static GstStaticPadTemplate sink_factory =
	GST_STATIC_PAD_TEMPLATE ("sink", GST_PAD_SINK, GST_PAD_ALWAYS,
	GST_STATIC_CAPS ("video/x-wmv, wmvversion = (int) 3, format=(fourcc)WMV3, fourcc=(fourcc)WMV3, width = (int) [ 16, 2048 ], height = (int) [ 16, 2048 ], framerate = (fraction) [ 0, MAX ]"
	";video/x-wmv, wmvversion = (int) {1,2}"));
#endif
...
gst_wmvdec_sinkpad_setcaps (GstPad *pad, GstCaps *caps)
{
	...
	gint ver = -1;
	gst_structure_get_int(str, "wmvversion", &ver);
	if(ver != 3) {
		GST_DEBUG_OBJECT(wmv, "wmvversion in x-wmv is %d, isn't supported by vMeta(only support 3)", ver);
		wmv->bForceForbidFormat = 1;
		return TRUE;	//just return TRUE, will terminate decoding in _chain();
	}
...
gst_wmvdec_chain (GstPad *pad, GstBuffer *buf)
{
	Gstwmvdec *wmv = GST_WMVDEC (GST_OBJECT_PARENT (pad));
	int ret = 0;
	if(wmv->bForceForbidFormat != 0) {
		gst_buffer_unref(buf);
		GST_ELEMENT_ERROR(wmv, STREAM, DECODE, (NULL), ("Terminate decoding, Not support this stream, reason %d", wmv->bForceForbidFormat));
