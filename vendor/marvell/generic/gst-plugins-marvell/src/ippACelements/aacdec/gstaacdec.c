...
IPP_auidoGetFrequencyIndex_AAC (int freq)
{
	...
	if ((freq == fs[index]) || index == 0) {
		return index;
	} else {
		if ((freq - fs[index]) > (fs[index-1] - freq)) {
			return (index - 1);
		} else {
			return index;
		}
...
/* modify with work around of v1 and v2(one stream contains both v1 and v2), it may come back after IPP enhaced */

#ifdef AAC_V1_WORKAROUND
	int version = 0;
	config.psEnable = 1;       /* Here must be 1 */
		
	if(IPP_STATUS_INIT_OK == (ret=DecoderInitAlloc_AAC(&(aacdec->srcBitStream), &(config), aacdec->pCBTable, &(aacdec->pAACDecState)))){
		
		/* work around */
		
		IppBitstream tmp_stream = aacdec->srcBitStream;
		Decode_AAC(&(tmp_stream), &(aacdec->sound), aacdec->pAACDecState);
		DecodeSendCmd_AACAudio(IPPAC_AAC_GET_VERSION, NULL, &version, aacdec->pAACDecState);

		if(version != 6){
			DecoderFree_AAC(&aacdec->pAACDecState);
			config.psEnable = 0;
			ret = DecoderInitAlloc_AAC(&(tmp_stream), &(config), aacdec->pCBTable, &(aacdec->pAACDecState));
		}		
	}
	return ret;
	
#else
	return DecoderInitAlloc_AAC(&(aacdec->srcBitStream), &(config), aacdec->pCBTable, &(aacdec->pAACDecState));
#endif
